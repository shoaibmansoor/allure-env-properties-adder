...
async function main(options, cli = false) {
    const args = cli ? getCliArgs(options) : options;
    const logger = new Logger(args.debug);
    logger.log(`Args from process: ${JSON.stringify(options)}`);

    // throw error if mandatory params are not provided
    optionsParser.validateArgs(args);

    // get file path
    const filePath = getTargetFile(args.outputDir || '.',
        args.fileName || ENV_FILE_NAME);
    logger.log(`FilePath: ${filePath}`);

    // create-write output data
    let data = '';
    Object.entries(args.props)
        .forEach(([key, value]) => data = `${data}${key}=${value}\n`);
...
function AllureEnvPropsAdder(properties) {
    ...
    self.jasmineDone = async function() {
